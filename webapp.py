...
@app.route('/auth/admin-browser-login', methods=['POST'])
@rate_limit(max_requests=10, window_seconds=60)
def admin_browser_login():
    """Handle Admin Browser authentication"""
    try:
        data = request.json
        ...
        # Check against Database Settings first, then Config
        db_instance = get_db()
        from settings_manager import SettingsManager
        settings_manager = SettingsManager(db_instance)
        
        # Try to get from DB
        stored_username = settings_manager.get_setting('admin_username')
        stored_password = settings_manager.get_setting('admin_password')
        
        # Fallback to config if not in DB
        if not stored_username:
            stored_username = WEBAPP_CONFIG.get('admin_username', 'admin')
        if not stored_password:
            stored_password = WEBAPP_CONFIG.get('admin_password')
        
        if not stored_password:
             return jsonify({'success': False, 'message': 'رمز عبور ادمین تنظیم نشده است. لطفا در تنظیمات (WEB_ADMIN_PASSWORD) را ست کنید.'}), 500

        if username == stored_username and password == stored_password:
             # Success - Login as Admin
             # We use the configured ADMIN_ID for the session
             admin_id = BOT_CONFIG.get('admin_id')
             
             db_user = db_instance.get_user(admin_id)
             
             # If admin user doesn't exist in DB, create/ensure it exists
             if not db_user:
                 try:
                     with db_instance.get_connection() as conn:
                         cursor = conn.cursor()
                         cursor.execute(
                             "INSERT INTO users (telegram_id, username, first_name, is_admin, balance) VALUES (%s, %s, %s, 1, 0) ON DUPLICATE KEY UPDATE is_admin=1",
                             (admin_id, stored_username, 'Admin')
                         )
                         conn.commit()
                         db_user = db_instance.get_user(admin_id)
                 except Exception as e:
             ...
             user_name = db_user.get('username', 'Admin') if db_user else 'Admin'
             first_name = db_user.get('first_name', 'System') if db_user else 'System'
             photo_url = ''
             try:
                photo_url = TelegramHelper.get_user_profile_photo_url_sync(admin_id)
             except:
                pass
            ...
            logger.warning(f"Failed browser admin login attempt for user: {username}")
            return jsonify({'success': False, 'message': 'نام کاربری یا رمز عبور اشتباه است'}), 401

    except Exception as e:
        logger.error(f"Error in Admin Browser auth: {e}")
        return jsonify({'success': False, 'message': 'خطا در احراز هویت'}), 500
...
def update_admin_credentials():
    """Update admin browser login credentials"""
    try:
        data = request.json
        ...
        if not new_username or not new_password:
            return jsonify({'success': False, 'message': 'نام کاربری و رمز عبور نمی‌توانند خالی باشند'}), 400
            
        if len(new_password) < 5:
            return jsonify({'success': False, 'message': 'رمز عبور باید حداقل ۵ کاراکتر باشد'}), 400
        ...
        # Save to database
        settings_manager.set_setting('admin_username', new_username)
        settings_manager.set_setting('admin_password', new_password)
        
        logger.info(f"Admin credentials updated by user {session.get('user_id')}")
        
        return jsonify({'success': True, 'message': 'اطلاعات ورود با موفقیت بروزرسانی شد'})
        
    except Exception as e:
        return secure_error_response(e, 'خطا در بروزرسانی اطلاعات')
